        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body pb-2">
                    ...
                    <form action="{{ route('questions.update', $question->id) }}" method="POST" enctype="multipart/form-data" novalidate>
                        @csrf
                        @method('PUT')
                        <div class="row">
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <div class="form-group">
                                    ...
                                    <select id="question_type" name="question_type" class="form-control" required>
                                        ...
                                        <option value="fill_in_the_blank_with_choice" {{ old('question_type', $question->question_type) == 'fill_in_the_blank_choice' ? 'selected' : '' }}>Fill in the Blank with Choice</option>
                                        <option value="fill_in_the_blank_with_text" {{ old('question_type', $question->question_type) == 'fill_in_the_blank_text' ? 'selected' : '' }}>Fill in the Blank with Text</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="question_text" class="form-control" placeholder="Enter the question text" required>{{ old('question_text', $question->question_text) }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="explanation" class="form-control" placeholder="Enter explanation for the answer">{{ old('explanation', $question->explanation) }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="is_active" class="form-control" required>
                                        ...
                                        <option value="active" {{ old('is_active', $question->is_active) == 'active' ? 'selected' : '' }}>Active</option>
                                        <option value="inactive" {{ old('is_active', $question->is_active) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                                        <option value="draft" {{ old('is_active', $question->is_active) == 'draft' ? 'selected' : '' }}>Draft</option>
                                    </select>
                                </div>
                            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const oldOptions = @json(old('options', json_decode($question->options ?? '[]')));
    let oldCorrectAnswers = @json(old('correct_answer', json_decode($question->correct_answer ?? '[]')));
    const oldQuestionType = @json(old('question_type', $question->question_type));
    ...
    // Initialize options with existing values
    if (oldQuestionType) {
